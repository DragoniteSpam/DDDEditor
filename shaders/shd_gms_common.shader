...
hey listen, youre going to need to re-hack this to use "custom" variables
(lights, fog, matrices, etc) instead of the ones built into game maker, probably
...
*/
//######################_==_YOYO_SHADER_MARKER_==_######################@~// Uniforms look like they're shared between vertex and fragment shaders in GLSL, so we have to be careful to avoid name clashes
/*
