...
if (Stuff.active_event!=noone){
    ...
    if (Stuff.event_node_info!=noone){
        ...
        draw_text(x1+16, y1+8, base.name);
        draw_line(x1+16, y1+24, x2-16, y1+24);
        if (n==0){
            draw_text_colour(x1+16, y1+40, "No data properties", c_gray, c_gray, c_gray, c_gray, 1);
        } else {
            for (var i=0; i<n; i++){
                ...
                draw_text(x1+16, y1+40+i*16, string(i+1)+". "+type[EventNodeCustomData.NAME]+" ("+type_name+")");
            }
        }
    }
}
